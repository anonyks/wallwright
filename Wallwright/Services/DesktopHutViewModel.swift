...
@MainActor
final class DesktopHutViewModel: ObservableObject {
    ...
    @Published var category: DesktopHutCategory = .trending
    @Published var items: [DesktopHutItem] = []
    ...
    var visibleItems: [DesktopHutItem] { items.filter { !hiddenItemIDs.contains($0.id) } }
    ...
    private let service = DesktopHutService()
    private static let hiddenIDsKey = "DesktopHutHiddenItemIDs"
    ...
    func loadCategory(_ category: DesktopHutCategory) {
    ...
    private func fetchCurrentPage() async throws -> [DesktopHutItem] {
        if isSearchActive {
    ...
    private func markAlreadyDownloaded() {
        ...
        let index = DesktopHutService.existingLibraryIndex(wallpapers: AppDelegate.shared.contentViewModel.wallpapers)
    ...
    func hide(_ item: DesktopHutItem) {
    ...
    func download(item: DesktopHutItem) {
