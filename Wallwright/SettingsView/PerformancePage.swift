...
struct PerformancePage: SettingsPage {
    ...
    /// `SystemUsageMonitor.sample()` spawns and waits on a `/bin/ps` subprocess — not free, and
    /// calling it directly from `.onAppear`/a button action blocked the main thread right as this
    /// page appeared or the refresh button was clicked. Dispatching it off-main and updating state
    /// on completion keeps the page itself responsive; `usageSnapshot` starting `nil` already reads
    /// as "Unavailable" while the first sample is still in flight, so no extra loading state needed.
