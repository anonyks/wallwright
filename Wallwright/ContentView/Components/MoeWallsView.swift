...
struct MoeWallsView: SubviewOfContentView {
    ...
    private var content: some View {
        if moeWallsVM.isLoading && moeWallsVM.items.isEmpty {
            ...
            Spacer()
            VStack(spacing: 10) {
                Image(systemName: moeWallsVM.isSearchActive ? "magnifyingglass" : "eye.slash")
                    .font(.system(size: 32))
                    .foregroundStyle(.tertiary)
                Text(moeWallsVM.isSearchActive ? "No results for \"\(moeWallsVM.searchQuery)\"" : "Everything here is hidden")
                    .font(.callout)
                    .foregroundStyle(.secondary)
            }
            .frame(maxWidth: 320)
