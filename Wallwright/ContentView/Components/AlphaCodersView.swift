...
struct AlphaCodersView: SubviewOfContentView {
    ...
    private var content: some View {
        if alphaCodersVM.isLoading && alphaCodersVM.items.isEmpty {
            ...
            Spacer()
            VStack(spacing: 10) {
                Image(systemName: alphaCodersVM.isSearchActive ? "magnifyingglass" : "eye.slash")
                    .font(.system(size: 32))
                    .foregroundStyle(.tertiary)
                Text(alphaCodersVM.isSearchActive ? "No results for \"\(alphaCodersVM.searchQuery)\"" : "Everything here is hidden")
                    .font(.callout)
                    .foregroundStyle(.secondary)
            }
            .frame(maxWidth: 320)
